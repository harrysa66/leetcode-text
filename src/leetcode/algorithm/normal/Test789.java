...
public class Test789 {
    ...
    public boolean escapeGhosts(int[][] ghosts, int[] target) {
        ...
        for (int i = 0; i < ghosts.length; i++) {
            int xLength = Math.abs(ghosts[i][0] - target[0]);
            int yLength = Math.abs(ghosts[i][1] - target[1]);
            int ghostLength = xLength + yLength;
            if(ghostLength <= targetLength){
                flag = false;
                break;
            }
        }
